...
export default function JuegosPage() {
  ...
  const handleRegister = async (game: RAWGGame) => {
    try {
      ...
      if (id) {
        const exists = registeredGames.find((g) => g.id === id);
        if (!exists) {
          saveGames([...registeredGames, { id, name: game.name, image_url: game.image_url }]);
        }
      }
    } catch (err: unknown) {
  ...
  const handleRemove = (id: number) => {
    saveGames(registeredGames.filter((g) => g.id !== id));
  };

  const registeredNames = new Set(registeredGames.map((g) => g.name));
      ...
      {searched && (
        <>
          <p style={{ fontSize: '13px', color: '#8892A4', marginBottom: '14px' }}>
            Resultados para "<span style={{ color: '#E2E8F0' }}>{query}</span>"
            {' — '}
            <span style={{ color: '#A78BFA', cursor: 'pointer' }} onClick={handleClearSearch}>
              volver a mis juegos
            </span>
          </p>

          {loading && <p style={{ color: '#8892A4', textAlign: 'center', padding: '32px' }}>Buscando...</p>}

          {!loading && results.length === 0 && (
            <p style={{ color: '#8892A4', textAlign: 'center', padding: '32px' }}>No se encontraron resultados</p>
          )}

          {!loading && results.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '32px' }}>
              {results.map((game) => {
                const already = registeredNames.has(game.name);
                const registeredId = registeredGames.find((g) => g.name === game.name)?.id;
                return (
      ...
      {!searched && (
        <>
          {registeredGames.length === 0 ? (
            ...
            <>
              <p style={{ fontSize: '13px', color: '#8892A4', marginBottom: '14px' }}>
                Mis juegos registrados ({registeredGames.length})
              </p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
                {registeredGames.map((game) => (
                  <div key={game.id} style={{ backgroundColor: '#0F1424', border: '1px solid #1E2540', borderRadius: '12px', overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '14px' }}>
                      <p style={{ fontSize: '14px', fontWeight: '700', color: '#FFFFFF', marginBottom: '8px' }}>{game.name}</p>
